...
vector<int> Solution::solve(int A, vector<vector<int> > &B){
 ...
 vector<int> ggArr(A, 0);// Create a vector ggArr of size A with  all values as 0.
int lI=0,rI=0;
    for(int i = 0; i < B.size(); i++) 
	{
		// -1 because vector start from 0 so initial value -1(B[i][0] - 1)
         lI = B[i][0] - 1,
         rI = B[i][1] - 1;
