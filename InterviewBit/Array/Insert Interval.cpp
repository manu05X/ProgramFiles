...
public:
    vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) 
    {
        ...
        {
            // All non overlapping interval
            res.push_back(intervals[i]);
            i++;
        }
        
        vector<int> merInt = newInterval;
        while(i < n && intervals[i][0] <= newInterval[1]) // All overlapping interval condition
        {    // All overlapping interval are merge
            merInt[0] = min(merInt[0], intervals[i][0]);
            merInt[1] = max(merInt[1], intervals[i][1]);
            i++;
        }
         res.push_back(merInt);
        
        while(i < n) // interval left 
        {
            res.push_back(intervals[i]);
            i++;
        }
